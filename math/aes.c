...
#define AES_SUBWORD(x)                                \
    AES_U32_LE( FORWARD_S_BOX[AES_BYTE0(x)],          \
                FORWARD_S_BOX[AES_BYTE1(x)],          \
                FORWARD_S_BOX[AES_BYTE2(x)],          \
                FORWARD_S_BOX[AES_BYTE3(x)] )

#define AES_SUBWORD_ROT(x)                            \
    AES_U32_LE( FORWARD_S_BOX[AES_BYTE1(x)],          \
                FORWARD_S_BOX[AES_BYTE2(x)],          \
                FORWARD_S_BOX[AES_BYTE3(x)],          \
                FORWARD_S_BOX[AES_BYTE0(x)] )
...
{
    memset( ctx, 0, sizeof( math_aes_context ) );
}
 ...
 */
int math_aes_setkey_enc( math_aes_context *ctx, const unsigned char *key,
                    unsigned int keybits )
{
    ...
    switch( keybits )
    {
        case 128: ctx->number_of_rounds = 10; break;
        case 192: ctx->number_of_rounds = 12; break;
        case 256: ctx->number_of_rounds = 14; break;
        default : return( -1 );
    }
...
int math_aes_setkey_dec( math_aes_context *ctx, const unsigned char *key,
                    unsigned int keybits )
{
    int i, j, ret;
...
#define AES_FORWARD_ROUND_WORD(RKP, A, B, C, D)     \
    ( *((RKP)++) ^                                  \
      AES_FORWARD_TABLE_0(AES_BYTE0(A)) ^           \
      AES_FORWARD_TABLE_1(AES_BYTE1(B)) ^           \
      AES_FORWARD_TABLE_2(AES_BYTE2(C)) ^           \
      AES_FORWARD_TABLE_3(AES_BYTE3(D)) )

#define AES_FORWARD_FINAL_WORD(RKP, A, B, C, D)     \
    ( *((RKP)++) ^                                  \
      AES_U32_LE( FORWARD_S_BOX[AES_BYTE0(A)],      \
                  FORWARD_S_BOX[AES_BYTE1(B)],      \
                  FORWARD_S_BOX[AES_BYTE2(C)],      \
                  FORWARD_S_BOX[AES_BYTE3(D)] ) )

int math_internal_aes_encrypt( math_aes_context *ctx,
                                  ...
                                  unsigned char output[16] )
{
    ...
    X0 = AES_GET_LE32(input, 0); X0 ^= *round_keys++;
    X1 = AES_GET_LE32(input, 4); X1 ^= *round_keys++;
    X2 = AES_GET_LE32(input, 8); X2 ^= *round_keys++;
    X3 = AES_GET_LE32(input, 12); X3 ^= *round_keys++;
...
#define AES_REVERSE_ROUND_WORD(RKP, A, B, C, D)     \
    ( *((RKP)++) ^                                  \
      AES_REVERSE_TABLE_0(AES_BYTE0(A)) ^           \
      AES_REVERSE_TABLE_1(AES_BYTE1(B)) ^           \
      AES_REVERSE_TABLE_2(AES_BYTE2(C)) ^           \
      AES_REVERSE_TABLE_3(AES_BYTE3(D)) )

#define AES_REVERSE_FINAL_WORD(RKP, A, B, C, D)     \
    ( *((RKP)++) ^                                  \
      AES_U32_LE( REVERSE_S_BOX[AES_BYTE0(A)],      \
                  REVERSE_S_BOX[AES_BYTE1(B)],      \
                  REVERSE_S_BOX[AES_BYTE2(C)],      \
                  REVERSE_S_BOX[AES_BYTE3(D)] ) )

int math_internal_aes_decrypt( math_aes_context *ctx,
                                  const unsigned char input[16],
                                  unsigned char output[16] )
{
    int i;
...
{
    { 0x9D, 0x1A, 0x35, 0x3C, 0x86, 0x7D, 0x41, 0xFB, 0x5B, 0x55, 0x38, 0x8D, 0x14, 0x9E, 0x84, 0xFD },
    { 0xB7, 0x6A, 0x17, 0x61, 0x47, 0x9E, 0x72, 0x9D, 0xAB, 0x88, 0x2D, 0xDD, 0x91, 0x01, 0x02, 0x9B },
    { 0x9E, 0x69, 0xA6, 0x57, 0xB0, 0x2D, 0x93, 0xD1, 0x20, 0x2F, 0x07, 0xBF, 0xF6, 0x77, 0x8D, 0xA1 }
};

static const unsigned char aes_test_ecb_enc[3][16] =
{
    { 0x47, 0x2B, 0x56, 0xF0, 0xBE, 0x7D, 0x8A, 0xE7, 0x72, 0x33, 0x47, 0x84, 0xE5, 0xCE, 0x29, 0x3E },
    { 0x35, 0x8D, 0xAE, 0x57, 0x83, 0x04, 0xC7, 0x71, 0x30, 0xBD, 0x0A, 0xDA, 0x76, 0x75, 0xD5, 0xCA },
    { 0x70, 0xDF, 0xF6, 0xF5, 0x0C, 0x1D, 0xE1, 0xA9, 0x9F, 0x55, 0xF1, 0x8D, 0xC2, 0x3B, 0xDF, 0xCF }
};
