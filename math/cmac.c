...
static void cmac_generate_subkeys(math_cmac_context_t *ctx, unsigned char k1[16],
                                  unsigned char k2[16])
{
...
static int cmac_starts(math_cmac_context_t *ctx, const unsigned char *key, size_t keybits)
{
    int ret;
...
static int cmac_update(math_cmac_context_t *ctx, const unsigned char *input, size_t ilen)
{
...
static int cmac_finish(math_cmac_context_t *ctx, unsigned char output[16])
{
    unsigned char k1[16];
    unsigned char k2[16];
    ...
    else
    {
        ...
        if (ctx->unprocessed_len > 0u)
        {
            memcpy(last_block, ctx->unprocessed_block, ctx->unprocessed_len);
        }
...
static int cmac_run_vector_suite(math_cmac_context_t *ctx, const cmac_test_suite_t *suite,
                                 const unsigned char *msg, const size_t msg_len[4],
                                 unsigned char out[16], int verbose)
{
    size_t i;
    int ret;
    ...
    for (i = 0u; i < 4u; i++)
    {
        ...
        ret = math_cmac_aes(ctx, suite->key, suite->keybits, msg, msg_len[i], out);
        if (ret != 0)
        {
            if (verbose != 0)
            {
        ...
        if (memcmp(out, suite->expected[i], 16u) != 0)
        {
            if (verbose != 0)
            {
                ...
                MACSEC_PRINT_HEX(("Expected CMAC", suite->expected[i], 16));
            }
...
static int cmac_run_streaming_test(math_cmac_context_t *ctx, const unsigned char *key,
                                   size_t keybits, const unsigned char *msg,
                                   const unsigned char expected[16], unsigned char out[16],
                                   int verbose)
{
    int ret;
    ...
    if (verbose != 0)
    {
        macsec_printf("    AES-%u streaming: ", (unsigned) keybits);
    }
...
int math_cmac_self_test(math_cmac_context_t *ctx, int verbose)
{
     ...
     */
    static const unsigned char msg[64] = {0x6Bu, 0xC1u, 0xBEu, 0xE2u, 0x2Eu, 0x40u, 0x9Fu, 0x96u,
    ...
    static const size_t msg_len[4] = {0u, 16u, 20u, 64u};

    static const unsigned char key_128[16] = {0x2Bu, 0x7Eu, 0x15u, 0x16u, 0x28u, 0xAEu,
                                              0xD2u, 0xA6u, 0xABu, 0xF7u, 0x15u, 0x88u,
                                              0x09u, 0xCFu, 0x4Fu, 0x3Cu};

    static const unsigned char expected_128[4][16] = {
        {0xBBu, 0x1Du, 0x69u, 0x29u, 0xE9u, 0x59u, 0x37u, 0x28u, 0x7Fu, 0xA3u, 0x7Du, 0x12u, 0x9Bu,
         0x75u, 0x67u, 0x46u},
        {0x07u, 0x0Au, 0x16u, 0xB4u, 0x6Bu, 0x4Du, 0x41u, 0x44u, 0xF7u, 0x9Bu, 0xDDu, 0x9Du, 0xD0u,
         0x4Au, 0x28u, 0x7Cu},
        {0x7Du, 0x85u, 0x44u, 0x9Eu, 0xA6u, 0xEAu, 0x19u, 0xC8u, 0x23u, 0xA7u, 0xBFu, 0x78u, 0x83u,
         0x7Du, 0xFAu, 0xDEu},
        {0x51u, 0xF0u, 0xBEu, 0xBFu, 0x7Eu, 0x3Bu, 0x9Du, 0x92u, 0xFCu, 0x49u, 0x74u, 0x17u, 0x79u,
         0x36u, 0x3Cu, 0xFEu}};

    static const unsigned char key_192[24] = {
        0x8Eu, 0x73u, 0xB0u, 0xF7u, 0xDAu, 0x0Eu, 0x64u, 0x52u, 0xC8u, 0x10u, 0xF3u, 0x2Bu,
        0x80u, 0x90u, 0x79u, 0xE5u, 0x62u, 0xF8u, 0xEAu, 0xD2u, 0x52u, 0x2Cu, 0x6Bu, 0x7Bu};

    static const unsigned char expected_192[4][16] = {
        {0xD1u, 0x7Du, 0xDFu, 0x46u, 0xADu, 0xAAu, 0xCDu, 0xE5u, 0x31u, 0xCAu, 0xC4u, 0x83u, 0xDEu,
         0x7Au, 0x93u, 0x67u},
        {0x9Eu, 0x99u, 0xA7u, 0xBFu, 0x31u, 0xE7u, 0x10u, 0x90u, 0x06u, 0x62u, 0xF6u, 0x5Eu, 0x61u,
         0x7Cu, 0x51u, 0x84u},
        {0x3Du, 0x75u, 0xC1u, 0x94u, 0xEDu, 0x96u, 0x07u, 0x04u, 0x44u, 0xA9u, 0xFAu, 0x7Eu, 0xC7u,
         0x40u, 0xECu, 0xF8u},
        {0xA1u, 0xD5u, 0xDFu, 0x0Eu, 0xEDu, 0x79u, 0x0Fu, 0x79u, 0x4Du, 0x77u, 0x58u, 0x96u, 0x59u,
         0xF3u, 0x9Au, 0x11u}};
    ...
    static const unsigned char expected_256[4][16] = {
        {0x02u, 0x89u, 0x62u, 0xF6u, 0x1Bu, 0x7Bu, 0xF8u, 0x9Eu, 0xFCu, 0x6Bu, 0x55u, 0x1Fu, 0x46u,
         0x67u, 0xD9u, 0x83u},
        {0x28u, 0xA7u, 0x02u, 0x3Fu, 0x45u, 0x2Eu, 0x8Fu, 0x82u, 0xBDu, 0x4Bu, 0xF2u, 0x8Du, 0x8Cu,
         0x37u, 0xC3u, 0x5Cu},
        {0x15u, 0x67u, 0x27u, 0xDCu, 0x08u, 0x78u, 0x94u, 0x4Au, 0x02u, 0x3Cu, 0x1Fu, 0xE0u, 0x3Bu,
         0xADu, 0x6Du, 0x93u},
        {0xE1u, 0x99u, 0x21u, 0x90u, 0x54u, 0x9Fu, 0x6Eu, 0xD5u, 0x69u, 0x6Au, 0x2Cu, 0x05u, 0x6Cu,
         0x31u, 0x54u, 0x10u}};

    static const cmac_test_suite_t suites[] = {{key_128, 128u, expected_128, "AES-128-CMAC"},
                                               {key_192, 192u, expected_192, "AES-192-CMAC"},
                                               {key_256, 256u, expected_256, "AES-256-CMAC"}};
    ...
    {
        ret = cmac_run_vector_suite(ctx, &suites[i], msg, msg_len, out, verbose);
        if (ret != 0)
        {
            goto fail;
        }
     ...
     */
    ret = cmac_run_streaming_test(ctx, key_128, 128u, msg, expected_128[3], out, verbose);
    if (ret != 0)
    {
        goto fail;
    }

    ret = cmac_run_streaming_test(ctx, key_256, 256u, msg, expected_256[3], out, verbose);
    if (ret != 0)
    {
        goto fail;
    }
