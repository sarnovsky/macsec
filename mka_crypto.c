...
static int macsec_mka_kdf(macsec_mka_crypto_ctx_t *ctx, const uint8_t *key, size_t key_len,
                          const char *label, const uint8_t *context, size_t context_len,
                          uint16_t out_bits, uint8_t *out, size_t out_len)
{
    ...
    MACSEC_MEDIUM(("MKA KDF: label='%s' key_len=%lu context_len=%lu out_bits=%u out_len=%lu\n",
                   label, (unsigned long) key_len, (unsigned long) context_len, out_bits,
    ...
    while (produced < out_len)
    {
        ...
        ret = macsec_mka_cmac(ctx, key, key_len, buf, buf_len, cmac_out);
...
static int macsec_mka_derive_one(macsec_mka_crypto_ctx_t *ctx, const char *label, uint8_t *out,
                                 size_t out_len)
{
    uint8_t keyid[16];
    uint16_t out_bits;
    ...
    macsec_check((out_len == 16u) || (out_len == 32u), MACSEC_ERR_PARAM);

    memset(keyid, 0, sizeof(keyid));

    memcpy(keyid, ctx->psk.ckn,
           (ctx->psk.ckn_len < sizeof(keyid)) ? ctx->psk.ckn_len : sizeof(keyid));
    ...
    ret = macsec_mka_kdf(ctx, ctx->psk.cak, ctx->psk.cak_len, label, keyid, sizeof(keyid), out_bits,
                         out, out_len);

    if (ret == MACSEC_ERR_OK)
    {
        MACSEC_MEDIUM_HEX(("MKA derived key", out, (int) out_len));
    }
 ...
 */
static int macsec_mka_aes_kw_wrap(macsec_mka_crypto_ctx_t *ctx, const uint8_t *kek, size_t kek_len,
                                  const uint8_t *plain, size_t plain_len, uint8_t *wrapped,
                                  size_t *wrapped_len, size_t wrapped_max_len)
{
    uint8_t a[8];
    uint8_t block[16];
    uint8_t b[16];
    ...
    MACSEC_MEDIUM(("MKA AES-KW wrap: plain_len=%lu wrapped_max_len=%lu\n",
                   (unsigned long) plain_len, (unsigned long) wrapped_max_len));
    ...
    for (j = 0u; j < 6u; j++)
    {
        for (i = 0u; i < n; i++)
        {
            ...
            ret = math_aes_crypt_ecb(&ctx->aes_ctx, MATH_AES_ENCRYPT, block, b);
            if (ret != 0)
            {
...
static int macsec_mka_aes_kw_unwrap(macsec_mka_crypto_ctx_t *ctx, const uint8_t *kek,
                                    size_t kek_len, const uint8_t *wrapped, size_t wrapped_len,
                                    uint8_t *plain, size_t *plain_len, size_t plain_max_len)
{
    uint8_t a[8];
    uint8_t block[16];
    uint8_t b[16];
    ...
    MACSEC_MEDIUM(("MKA AES-KW unwrap: wrapped_len=%lu plain_max_len=%lu\n",
                   (unsigned long) wrapped_len, (unsigned long) plain_max_len));
    ...
    for (j_rev = 6u; j_rev > 0u; j_rev--)
    {
        ...
        for (i = n; i > 0u; i--)
        {
            ...
            ret = math_aes_crypt_ecb(&ctx->aes_ctx, MATH_AES_DECRYPT, block, b);
            if (ret != 0)
            {
...
int macsec_mka_crypto_derive_ick_kek(macsec_mka_crypto_ctx_t *ctx)
{
    ...
    ret = macsec_mka_derive_one(ctx, "IEEE8021 KEK", ctx->keys.kek, derived_len);
    if (ret != MACSEC_ERR_OK)
    {
    ...
    MACSEC_MEDIUM_HEX(("MKA ICK", ctx->keys.ick, ctx->keys.ick_len));

    MACSEC_MEDIUM_HEX(("MKA KEK", ctx->keys.kek, ctx->keys.kek_len));
                               ...
                               uint8_t mic[MACSEC_MKA_MIC_LEN])
{
    ...
    return macsec_mka_cmac(ctx, ctx->keys.ick, ctx->keys.ick_len, pdu, pdu_len, mic);
}
                               ...
                               size_t wrapped_sak_max_len)
{
    ...
    return macsec_mka_aes_kw_wrap(ctx, ctx->keys.kek, ctx->keys.kek_len, sak, sak_len, wrapped_sak,
                                  wrapped_sak_len, wrapped_sak_max_len);
}
                                 ...
                                 size_t sak_max_len)
{
    ...
    ret = macsec_mka_aes_kw_unwrap(ctx, ctx->keys.kek, ctx->keys.kek_len, wrapped_sak,
                                   wrapped_sak_len, sak, sak_len, sak_max_len);
...
{
    static const uint8_t cak[16] = {0x00u, 0x11u, 0x22u, 0x33u, 0x44u, 0x55u, 0x66u, 0x77u,
                                    0x88u, 0x99u, 0xAAu, 0xBBu, 0xCCu, 0xDDu, 0xEEu, 0xFFu};

    static const uint8_t ckn[16] = {0x10u, 0x11u, 0x12u, 0x13u, 0x14u, 0x15u, 0x16u, 0x17u,
    ...
    ret = macsec_mka_crypto_set_psk(&test_ctx->ctx, cak, sizeof(cak), ckn, sizeof(ckn));
    if (ret != MACSEC_ERR_OK)
    {
        goto fail;
    }
    ...
    ret = macsec_mka_crypto_calc_mic(&test_ctx->ctx, test_ctx->pdu, 96u, test_ctx->mic);
    if (ret != MACSEC_ERR_OK)
    {
        goto fail;
    }

    ret = macsec_mka_crypto_verify_mic(&test_ctx->ctx, test_ctx->pdu, 96u, test_ctx->mic);
    if (ret != MACSEC_ERR_OK)
    {
        goto fail;
    }
    ...
    ret = macsec_mka_crypto_wrap_sak(&test_ctx->ctx, test_ctx->sak, 16u, test_ctx->wrapped,
                                     &wrapped_len, sizeof(test_ctx->wrapped));
    if (ret != MACSEC_ERR_OK)
    {
        goto fail;
    }

    ret = macsec_mka_crypto_unwrap_sak(&test_ctx->ctx, test_ctx->wrapped, wrapped_len,
                                       test_ctx->unwrapped, &unwrapped_len,
